...
grafik_minyak_tertinggi = st.container()
with grafik_minyak_tertinggi:
    st.markdown('***')
    st.markdown('### Grafik B-besar negara dengan jumlah produksi terbesar secara kumulatif keseluruhan tahun')
    try:
        # Input untuk jumlah peringkat
        inputrank = st.number_input('Jumlah negara', value=5, step=1)
        jumlahproduksi = (dfM[['kode_negara', 'produksi']].groupby('kode_negara', as_index=False).sum().sort_values(['produksi'], ascending=[0])).reset_index(drop=True)
        jumlahproduksi = jumlahproduksi[:int(inputrank)].reset_index(drop=True)
        jumlahproduksiOut = jumlahproduksi[['kode_negara', 'produksi']].rename(columns={'kode_negara':'index'}).set_index('index')
        grafik_tabel(jumlahproduksiOut)
    except Exception:
        st.write('Ada kesalahan dalam input anda')
